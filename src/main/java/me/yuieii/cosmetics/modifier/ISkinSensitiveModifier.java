...
public interface ISkinSensitiveModifier {
    default boolean isApplicable(AbstractClientPlayerEntity player) {
        OptionalBoolean preCheck = this.isPlayerApplicable(player);
        if (preCheck.isPresent()) {
            return preCheck.get();
        }

        ResourceLocation skinLocation = player.getSkinTextureLocation(); // .skin.texture();
        TextureManager manager = Minecraft.getInstance().getTextureManager();
        Texture texture = manager.getTexture(skinLocation);
        if (!(texture instanceof SimpleTexture)) {
            return false;
        }

        SimpleTexture simple = (SimpleTexture) texture;
    ...
    default OptionalBoolean isPlayerApplicable(AbstractClientPlayerEntity player) {
    ...
    static RuntimeException createIllegalPositionArrayArgumentException() {
        return new IllegalArgumentException("Invalid position array. Must have { x1, y1, x2, y2, ... } structure.");
    }

    static boolean colorsAllMatch(int[] positions, NativeImage bitmap, Predicate<Color> predicate) {
